...
export function initTracker(opts) {
  ...
  var api = {
    ...
    async login(pw) {
      ...
      if (r.ok && j.ok) { admin = true; return { ok: true }; }
      return { ok: false, error: j.error || "Incorrect password." };
    },
    async logout() { await fetch("/api/logout", { method: "POST" }).catch(function () {}); admin = false; },
    async changePassword(cur, nw) {
      var r = await fetch("/api/password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ current: cur, new: nw }) });
      var j = await r.json().catch(function () { return {}; });
      return r.ok && j.ok ? { ok: true } : { ok: false, error: j.error || "Could not update password." };
    },
    async upload(buf) {
      var r = await fetch("/api/upload", { method: "POST", headers: { "Content-Type": "application/octet-stream" }, body: buf });
      var j = await r.json().catch(function () { return {}; });
      return r.ok && j.ok ? { ok: true, dataset: j.dataset, stats: j.stats } : { ok: false, error: j.error || "Upload failed." };
    },
    async reset(mode) {
      var r = await fetch("/api/upload?reset=" + mode, { method: "POST" });
      var j = await r.json().catch(function () { return {}; });
      return r.ok && j.ok ? { ok: true } : { ok: false, error: j.error || "Reset failed." };
    },
  ...
  function footer(m) { return '<div style="font-size:11px;color:#A89C97;text-align:center;margin:22px 0 8px">VMBS ' + esc(m.label) + ' · Source: ' + esc(m.source) + ' · ' + m.branches.length + ' branches</div>'; }
  ...
  function renderHeatmap() {
    ...
    document.getElementById("content").innerHTML =
      filterChips(m) +
      '<div class="h2l">' + esc(metric.label) + ' heatmap</div><div class="chips" style="margin-bottom:8px">' + metricChips + "</div>" +
  ...
  function renderReport() {
    ...
    var band = (m.headline.band || defectBand)(hv);
    var hfmt = m.kind === "opstd" ? fmtScore(hv) : fmtPct(hv);
    var fmtV = function (v) { return v == null ? "—" : (m.kind === "opstd" ? fmtScore(v) : fmtPct(v)); };

    // Branches ranked best → worst on the headline metric.
    var per = m.branches.map(function (name, i) {
      return { i: i, name: name, v: m.aggregate(m.filterRows(assign(f, { branch: i })), m.headline.key) };
    }).filter(function (x) { return x.v != null; });
    per.sort(function (x, y) { return m.headline.betterHigh ? y.v - x.v : x.v - y.v; });

    var now = new Date();
    var genDate = MON3[now.getMonth()] + " " + now.getDate() + ", " + now.getFullYear();
    var yearLabel = state.year === "all" ? m.years[0] + "–" + m.years[m.years.length - 1] : state.year;
    ...
    var sb = m.secondaryBreakdown(f);
    var sbRows = sb.cards.map(function (c, i) {
      return '<div class="row between mid" style="padding:10px 0;gap:10px' + (i ? ";border-top:1px solid #F1ECE9" : "") + '">' +
        '<div style="flex:1;min-width:0"><div class="t13">' + esc(c.name) + '</div><div class="sub" style="margin-top:2px">' + esc(c.sub) + '</div></div>' +
        '<span class="statpill" style="background:' + c.band.bg + ';color:' + c.band.c + '">' + c.value + '</span></div>';
    }).join("");

    // Ranked branch leaderboard (only meaningful when not scoped to one branch).
    var branchSection = "";
    if (state.branch < 0 && per.length > 1) {
      var ranked = per.map(function (x, idx) {
        var b = (m.headline.band || defectBand)(x.v);
        return '<div class="row between mid" style="padding:9px 0;gap:10px' + (idx ? ";border-top:1px solid #F1ECE9" : "") + '">' +
          '<div class="row mid gap8" style="flex:1;min-width:0"><span style="font-family:Sora;font-size:11px;font-weight:800;color:var(--dim);min-width:18px;text-align:right">' + (idx + 1) + '</span>' +
          '<div class="t13" style="min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(x.name) + '</div></div>' +
          '<span class="statpill" style="background:' + b.bg + ';color:' + b.c + '">' + fmtV(x.v) + '</span></div>';
      }).join("");
      branchSection = '<div class="h2l">Branch ranking · ' + esc(m.headline.label.toLowerCase()) + '</div>' +
        '<div class="sub13" style="margin:0 4px 6px">All ' + per.length + ' scored branches, best to worst.</div>' +
        '<div class="card">' + ranked + '</div>';
    }

    document.getElementById("content").innerHTML =
      filterChips(m) +
      '<div class="card" style="margin-bottom:12px"><div class="row between mid" style="gap:10px">' +
        '<div style="flex:1;min-width:0"><div class="t15">' + esc(m.label) + ' report</div>' +
        '<div class="sub13" style="margin-top:4px">' + esc(scopeLabel(m)) + '</div>' +
        '<div class="sub" style="margin-top:4px">Generated ' + esc(genDate) + ' · Source: ' + esc(m.source) + '</div></div>' +
        '<button class="lockbtn no-print" data-action="print" title="Print or save as PDF" aria-label="Print report">' +
          '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9V2h12v7"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg></button></div></div>' +
      '<div class="hero" style="margin-bottom:6px"><div class="row between base"><span class="eyebrow">' + esc(m.headline.label.toUpperCase()) + '</span><span class="dim12">' + esc(yearLabel) + '</span></div>' +
        '<div class="big">' + hfmt + '</div><div class="dim13">' + esc(band.label) + '</div></div>' +
      '<div class="h2l">Executive summary</div><div class="card"><div class="sub13">' + narr + '</div></div>' +
      '<div class="grid-status">' + status + '</div>' +
      '<div class="h2l">' + esc(m.headline.label) + ' trend</div><div class="card">' + sparkline(m) + '</div>' +
      '<div class="h2l">' + esc(sb.title) + '</div><div class="card">' + sbRows + '</div>' +
      branchSection + footer(m);
  }
  ...
  // ---- CSV export (client-side, current filtered scope) ----
  function exportCSV() {
    var m = model(), f = filters(), lines, name;
    if (m.kind === "defects") {
      ...
      m.filterRows(f).forEach(function (r) { lines.push([m.periods[r[0]].iso, '"' + m.branches[r[1]] + '"'].concat(m.metrics.map(function (x) { return r[2 + x.key] == null ? "" : r[2 + x.key]; })).join(",")); });
      name = "operational-standard";
    }
    var blob = new Blob([lines.join("\n")], { type: "text/csv" });
    var url = URL.createObjectURL(blob), a = document.createElement("a");
    a.href = url; a.download = name + "-" + state.year + ".csv"; a.click(); URL.revokeObjectURL(url);
  }
  ...
  return function teardown() {
    ...
    if (exportBtn) exportBtn.removeEventListener("click", exportCSV);
  };
}
