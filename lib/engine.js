...
export function initTracker(opts) {
  ...
  // ---- Operational Standard + PMAS block for the combined branch report ----
  function opStdRatingOf(v) { return v == null ? null : (v >= 95 ? 5 : v >= 80 ? 4 : v >= 70 ? 3 : v >= 60 ? 2 : 1); }
  function opStdPmasBlock(bi, iso, full) {
    ...
    if (!target) return "";
    var pi = target.idx, mnum = parseInt(target.p.iso.slice(5, 7), 10);
    var overall = om.cellValue(bi, pi, 0), rating = opStdRatingOf(overall), ob = scoreBand(overall);
    ...
    var comps = [[1, "Average Customer SLA"], [2, "Queue SLA"], [3, "Onboarding SLA"], [4, "Procurement"], [5, "Compliance to Major Procedure"], [7, "Complaints Resolved"], [8, "Audit Resolution"]];
    var compHtml = full ? '<div class="card" style="margin:12px 0 0;padding:4px 14px;box-shadow:none;border:1px solid var(--line)">' + comps.map(function (c) { var v = om.cellValue(bi, pi, c[0]); return '<div class="row between mid" style="padding:7px 0;border-bottom:1px solid #F4EFED"><span class="sub" style="color:#6E625E">' + esc(c[1]) + '</span><span class="num16">' + (v == null ? "—" : fmtScore(v)) + "</span></div>"; }).join("") + "</div>" : "";
    return '<hr class="report-rule">' +
      '<div class="report-h">Operational Standard · ' + esc(target.p.full) + "</div>" + verdict(overall, rating, ob) + compHtml +
      '<div class="report-h" style="margin-top:20px">PMAS to date · ' + basis + " basis</div>" + verdict(pmas, opStdRatingOf(pmas), pb) +
      '<div class="sub13" style="margin-top:6px">Running average of the branch’s monthly Operational Standard score — ' + vals.length + " of " + div + " months in. " + basis + " PMAS divides the sum by " + div + ".</div>";
  }
  ...
  function renderReport() {
    ...
    document.getElementById("content").innerHTML =
      ...
      '<div class="report">' +
        ...
        (state.branch >= 0 ? opStdPmasBlock(state.branch, repPi >= 0 ? m.periods[repPi].iso : null, m.kind === "defects") : "") +
      "</div>" + footer(m);
  }
