...
export function initTracker(opts) {
  ...
  var state = {
    ...
    entry: null, subs: null, subsErr: "", reviewId: null,
  };
  var savedYear = null;
  ...
  function subRowHtml(s, kind) {
    ...
    if (kind === "returned") extra = '<div class="msgbox info" style="margin-top:10px">' + esc(s.note ? "“" + s.note + "” — " + (s.supervisor || "supervisor") : "Returned for changes.") + "</div>" +
      '<button class="cta2" data-action="edit-sub" data-id="' + s.id + '" style="margin-top:8px;color:var(--red);border-color:#F0CDD2">Fix &amp; resubmit</button>';
    ...
    else if (kind === "submitted") extra = '<div class="sub" style="margin-top:8px">With ' + esc(s.supervisor || "your supervisor") + " · sent " + esc(timeAgo(s.history && s.history.length ? s.history[s.history.length - 1].at : s.updatedAt)) + "</div>";
    else if (kind === "published") extra = '<div class="row between mid" style="margin-top:8px"><span class="sub">Approved by ' + esc(s.supervisor || "supervisor") + "</span><button class=\"chip\" data-action=\"goto-published\" data-id=\"" + s.id + '">View in Pulse →</button></div>';
  ...
  function renderReview() {
    ...
    if (state.subsErr) { content.innerHTML = '<div class="card"><div class="errmsg" style="margin:0">' + esc(state.subsErr) + "</div></div>"; return; }
    var queue = state.subs.filter(function (s) { return s.status === "submitted"; }).sort(function (a, b) { return (a.history && a.history.length ? a.history[a.history.length - 1].at : a.updatedAt).localeCompare(b.history && b.history.length ? b.history[b.history.length - 1].at : b.updatedAt); });
    var reviewed = state.subs.filter(function (s) { return s.status === "published" || s.status === "returned"; }).slice(0, 6);
    var rows = queue.map(function (s) {
      var when = s.history && s.history.length ? s.history[s.history.length - 1].at : s.updatedAt;
      var h = reviewHeadline(s);
      return '<button class="card btncard" data-action="open-review" data-id="' + s.id + '" style="margin-bottom:10px">' +
        '<div class="row between mid"><span class="sub"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#8A7E7A" stroke-width="2" style="vertical-align:-1px;margin-right:3px"><circle cx="12" cy="8" r="4"/><path d="M4 21v-1a6 6 0 0 1 12 0v1"/></svg>From <b style="color:var(--ink)">' + esc(s.inspector) + '</b></span><span class="sub">' + esc(timeAgo(when)) + "</span></div>" +
    ...
    content.innerHTML =
      '<div class="row between mid" style="margin:6px 4px 12px"><div class="h2l" style="margin:0">Awaiting sign-off</div><span class="statpill" style="background:var(--red);color:#fff">' + queue.length + "</span></div>" +
      (queue.length ? rows : '<div class="card awaiting"><div class="t15" style="margin-bottom:4px">Queue clear</div><div class="sub13">No submissions waiting. New ones from your inspectors will appear here.</div></div>') +
      (reviewed.length ? '<div class="h2l" style="margin-top:20px">Recently reviewed</div>' + reviewed.map(function (s) {
        return '<div class="card" style="margin-bottom:10px"><div class="row between mid"><div style="min-width:0;flex:1;padding-right:8px"><div class="t13">' + subTitle(s) + '</div><div class="sub" style="margin-top:3px">From ' + esc(s.inspector) + " · " + esc(periodFull(s.dataset, s.period)) + "</div></div>" + statusPill(s.status) + "</div></div>";
      }).join("") : "") +
      '<div class="metabar" style="margin-top:14px;color:#8A7E7A;font-size:11px">Approving publishes into the live dataset — Pulse, Heatmap, Register and Report update at once.</div>';
  }
  function renderReviewDetail() {
    ...
    if (s.dataset === "defects") {
      valRows = DEFECT_INPUTS.map(function (f) { return '<div class="row between mid" style="padding:9px 0;border-bottom:1px solid #F4EFED"><span class="t13" style="font-weight:600">' + esc(f[1]) + '</span><span class="num16">' + fmtNum(+s.values[f[0]] || 0) + "</span></div>"; }).join("");
    } else {
      valRows = (m.metrics || []).map(function (mm, i) { var v = s.values[i]; return '<div class="row between mid" style="padding:9px 0;border-bottom:1px solid #F4EFED"><span class="t13" style="font-weight:600">' + esc(mm.label) + '</span><span class="num16">' + (v == null || v === "" ? "—" : fmtScore(+v)) + "</span></div>"; }).join("") +
    ...
    // Impact preview: current published value for this exact cell vs the submitted value.
    var impact = "";
    if (s.dataset === "defects") {
      ...
      impact = line("Defect rate · " + periodFull(s.dataset, s.period), cur.defectRate, nw.defectRate, defectBand) +
      ...
      impact = '<div class="row between mid" style="padding:7px 0"><span class="sub">Op Standard Score · ' + esc(periodFull(s.dataset, s.period)) + '</span><span class="row mid gap8"><span class="sub" style="color:#B9AEA8">' + (curOv == null ? "—" : fmtScore(curOv)) + '</span><span style="color:#8A7E7A">→</span><span class="statpill" style="background:' + bnd.bg + ";color:" + bnd.c + '">' + (nwOv == null || nwOv === "" ? "—" : fmtScore(+nwOv)) + "</span></span></div>" +
    ...
    content.innerHTML =
      ...
      '<div class="h2l">Values submitted</div><div class="card" style="margin-bottom:10px;padding:4px 14px">' + valRows + "</div>" +
      '<div class="h2l">If published — impact on ' + esc(s.branch) + "</div><div class=\"card\" style=\"margin-bottom:10px\">" + impact + "</div>" +
  ...
  // ---- Events ----
  function onClick(e) {
    var t = e.target.closest("[data-mode],[data-tab],[data-branch],[data-area],[data-heat],[data-b][data-p],[data-ba][data-aa],[data-sort],[data-action],[data-dlfmt],[data-stop]");
    if (!t) return;
