/* Pulse Quality tracker — rendering engine.
...
export function initTracker(opts) {
  ...
  var STORAGE = "pulse-quality-tracker:ui";
  ...
  function renderHeatmap() {
    ...
    document.getElementById("content").innerHTML =
      ...
      '<div class="card" style="overflow-x:auto;padding:12px"><div style="min-width:' + (NAME_W + months.length * (CELL_W + 2)) + 'px">' + headerYears + headerMonths + rows + "</div></div>" + footer(m);
    renderHeatDetail();
  }
  function renderHeatDetail() {
    ...
    el.innerHTML = '<div class="notebox" style="margin-bottom:10px"><div class="b t13">' + esc(m.branches[b]) + " · " + esc(m.periods[p].full) + "</div><div class=\"row gap8 wrap\" style=\"margin-top:6px\">" + m.cellDetail(b, p, filters()) + "</div></div>";
  }
