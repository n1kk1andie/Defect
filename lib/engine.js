...
export function initTracker(opts) {
  ...
  function renderHeatmap() {
    ...
    var desc = areaView
      ? "Each cell is a branch × process area — the top figure is its " + esc(metric.label.toLowerCase()) + ", the small figure the counts behind it (" + esc(metric.noun) + " ÷ base). A ▲/▼ marks the change from the previous year — green improved, red worsened, grey if the rate held but the volume moved. Branches run best to worst, top to bottom; tap a cell for the full breakdown."
