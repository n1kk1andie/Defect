...
export function initTracker(opts) {
  ...
  // Whether the server can persist changes (Vercel Blob connected, or non-Vercel
  // host). When false, in-app password changes won't survive the next request.
  var canPersist = opts.canPersist !== false;
  ...
  var state = {
    ...
    heatView: "time", heatSel: null, ui: { msg: "", msgKind: "", pwMsg: "", pwOk: false }, pendingTab: null,
  };
  var savedYear = null;
  try { var saved = JSON.parse(localStorage.getItem(STORAGE) || "{}"); ["mode", "tab"].forEach(function (k) { if (saved[k] != null) state[k] = saved[k]; }); if (saved.year != null) savedYear = saved.year; if (saved.heatMetric) state.heatMetric = saved.heatMetric; if (saved.heatView === "time" || saved.heatView === "area") state.heatView = saved.heatView; } catch (e) {}
  function persist() { try { localStorage.setItem(STORAGE, JSON.stringify({ mode: state.mode, tab: state.tab, year: state.year, heatMetric: state.heatMetric, heatView: state.heatView })); } catch (e) {} }
  ...
  function filterChips(m) {
    ...
    var areaAsAxis = m.hasArea && state.tab === "heatmap" && state.heatView === "area";
  ...
  // ---- Screen: Heatmap ----
  // Two lenses on the same data. "Over time" (branch × month) shows *where* and
  // *when* a metric runs hot. "By process area" (branch × process area) shows
  // *what* is driving it and *how bad* — each cell carries the rate and the raw
  // counts, so root-cause reads straight off the grid.
  function renderHeatmap() {
    var m = model(), mk = state.heatMetric[state.mode], metric = m.metric(mk), f = filters();
    var areaView = m.hasArea && state.heatView === "area";
    ...
    var viewToggle = m.hasArea
      ? '<div class="chips" style="margin-bottom:8px">' +
          '<button class="chip' + (!areaView ? " on" : "") + '" data-hview="time">Over time</button>' +
          '<button class="chip' + (areaView ? " on" : "") + '" data-hview="area">By process area</button>' +
        "</div>"
      : "";
    var desc = areaView
      ? "Each cell is a branch × process area — the top figure is its " + esc(metric.label.toLowerCase()) + ", the small figure the counts behind it (" + esc(metric.noun) + " ÷ base). Branches run best to worst, top to bottom; tap a cell for the full breakdown."
  ...
  function heatAreaGrid(m, mk, metric, f) {
    ...
    var rows = orderBranches(m, mk, metric, f).map(function (o) {
      ...
      var cells = areas.map(function (aname, ai) {
        ...
        return '<div class="hmac" data-ba="' + bi + '" data-aa="' + ai + '" style="width:' + AREA_W + "px;height:" + AREA_H + "px;background:" + pal.bg + ";color:" + pal.c + (selq ? ";outline:2px solid var(--ink);outline-offset:-2px" : "") + '"><span class="r">' + fmtPct(c.rate) + '</span><span class="c">' + sub + "</span></div>";
      }).join("");
      return '<div style="display:flex;align-items:center;margin-bottom:2px"><div class="hmname" style="width:' + NAME_W + 'px">' + esc(o.bname) + "</div>" + cells + "</div>";
    }).join("");
  ...
  function renderHeatDetail() {
    ...
    var m = model(), f = filters();
    if (m.hasArea && state.heatView === "area" && sel.a != null) {
  ...
  function renderReport() {
    ...
    if (!isAdmin()) {
      document.getElementById("content").innerHTML =
        ...
        '<div class="t15" style="margin-bottom:6px">Admin sign-in required</div><div class="sub13" style="max-width:320px;margin:0 auto 14px">The report — executive summary, branch table and data downloads — is available to admins.</div>' +
        '<button class="cta" style="max-width:220px;margin:0 auto" data-action="login">Sign in as admin</button></div>';
      return;
    }
    var f = filters();
    var rows = m.filterRows(f);
  ...
  function renderSettings() {
    ...
    // When the server can't persist changes, changing the password won't stick —
    // warn up front and point to the fixes instead of letting it silently revert.
    var noPersistWarn = canPersist ? "" : '<div class="msgbox info" style="margin-bottom:12px">Changes can’t be saved on this deployment (no persistent storage). To change the password, connect a Vercel Blob store to the project, or set the <b>ADMIN_PASSWORD</b> environment variable and redeploy.</div>';
    ...
    document.getElementById("content").innerHTML =
      ...
      '<div class="card" style="margin-bottom:10px">' + noPersistWarn + '<div class="sub13" style="margin-bottom:12px">Change the password used to sign in as admin. It is hashed and stored server-side and applies to every browser.</div>' +
  ...
  var TABS = [
    ...
    { id: "report", label: "Report", admin: true, icon: "M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8zM14 2v6h6M16 13H8M16 17H8M10 9H8" },
  ...
  // (no popup). Report is admin-gated, so prompt for sign-in when needed.
  function openDownloads() {
    var tdef = TABS.filter(function (x) { return x.id === "report"; })[0];
    if (tdef && tdef.admin && !isAdmin()) { state.pendingTab = "report"; showLoginModal(); return; }
  ...
  async function handleAction(a) {
    ...
    if (a === "logout") { await api.logout(); var ct = TABS.filter(function (x) { return x.id === state.tab; })[0]; if (ct && ct.admin) state.tab = "pulse"; render(); return; }
  ...
  // ---- Events ----
  function onClick(e) {
    var t = e.target.closest("[data-mode],[data-tab],[data-branch],[data-area],[data-heat],[data-hview],[data-b][data-p],[data-ba][data-aa],[data-sort],[data-action],[data-dlfmt],[data-stop]");
    if (!t) return;
    ...
    if (t.hasAttribute("data-tab")) {
      ...
      var tdef = TABS.filter(function (x) { return x.id === tab; })[0];
      if (tdef && tdef.admin && !isAdmin()) { state.pendingTab = tab; showLoginModal(); return; }
    ...
    if (t.hasAttribute("data-hview")) { var hv = t.getAttribute("data-hview"); if (state.heatView !== hv) { state.heatView = hv; if (hv === "area") state.area = -1; state.heatSel = null; } persist(); render(); return; }
