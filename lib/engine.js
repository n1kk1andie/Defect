...
export function initTracker(opts) {
  ...
  function buildDefects() {
    ...
    function statusCards(rows) {
      var a = record(rows);
      return [{ value: fmtNum(a.reviewed), label: "Files reviewed" }, { value: fmtNum(a.defects), label: "Defects found", color: "#8E0E1F", tap: 'data-action="def-cell"' },
  ...
  function buildOpStd() {
    ...
    function extraSection(f) {
      // The audit-grades chart is a historical trend, so it carries its own range
      // toggle (All years / current year) independent of the page's year filter —
      // it defaults to all years so the full 2024–2026 history is visible, while the
      // rest of Pulse stays scoped to the selected year.
      var m = this;
      var auditAll = state.auditAll !== false;
      var focusYear = state.year === "all" ? m.years[m.years.length - 1] : state.year;
      var months = m.periods.map(function (p, pi) { return { p: p, pi: pi }; }).filter(function (x) {
        return x.pi <= m.lastActive && (auditAll || x.p.year === focusYear);
      });
      var rangeToggle = '<div class="chips no-print" style="margin:0 4px 10px">' +
        '<button class="chip' + (auditAll ? " on" : "") + '" data-audit="all">All years</button>' +
        '<button class="chip' + (!auditAll ? " on" : "") + '" data-audit="year">' + esc(focusYear) + "</button></div>";
      ...
      return '<div class="h2l">Audit grades by month</div>' +
        '<div class="sub13" style="margin:0 4px 10px">Letter grade derived from the audit score each month, stacked across branches (2024 used a 1–5 scale; 2025+ used A–D). Number on each bar is branches graded that month.</div>' + rangeToggle + legend +
  ...
  // Whether Microsoft SSO is configured on this deployment. When true, the login
  // modal shows a "Sign in with Microsoft" button alongside the role/admin
  // sign-in. Inert (button hidden) when false — nothing else changes.
  var SSO_ENABLED = !!opts.ssoEnabled;
  var SSO_ERRORS = {
    not_authorized: "That Microsoft account isn’t recognised for Pulsus Quality. Ask an administrator to add you.",
    sso: "Microsoft sign-in isn’t available right now.",
    sso_state: "Microsoft sign-in didn’t complete — please try again.",
    sso_token: "Microsoft sign-in didn’t complete — please try again.",
    sso_email: "Couldn’t read your email from Microsoft — please try again."
  };
  ...
  var api = {
    ...
    async uploadAttachment(file) {
      ...
      var res = await apiCall("/api/attachment", { method: "POST", body: fd }); // no Content-Type: browser sets the multipart boundary
      if (res.ok && res.json.ok) return { ok: true, attachment: res.json.attachment };
      return { ok: false, error: res.status === 0 ? NET_ERR : (res.json.error || "Upload failed.") };
    },
    async resolveDefects(refs, resolved) {
      var res = await apiCall("/api/submissions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "resolve", refs: refs, resolved: resolved }) });
      if (res.ok && res.json.ok) return { ok: true, submissions: res.json.submissions, changed: res.json.changed };
      return { ok: false, error: res.status === 0 ? NET_ERR : (res.json.error || "Could not update resolution.") };
    },
  ...
  var state = {
    ...
    auditAll: true, histYear: "all",
  };
  var savedYear = null;
  try { var saved = JSON.parse(localStorage.getItem(STORAGE) || "{}"); ["mode", "tab"].forEach(function (k) { if (saved[k] != null) state[k] = saved[k]; }); if (saved.year != null) savedYear = saved.year; if (saved.heatMetric) state.heatMetric = saved.heatMetric; if (saved.auditAll != null) state.auditAll = saved.auditAll; if (saved.histYear != null) state.histYear = saved.histYear; } catch (e) {}
  function persist() { try { localStorage.setItem(STORAGE, JSON.stringify({ mode: state.mode, tab: state.tab, year: state.year, heatMetric: state.heatMetric, auditAll: state.auditAll, histYear: state.histYear })); } catch (e) {} }
  function model() { return MODELS[state.mode]; }
  // The History (heatmap) tab keeps its own year selection, defaulting to all years,
  // so it opens on the full timeline while the other tabs stay scoped to the chosen year.
  function yearFor() { return state.tab === "heatmap" ? state.histYear : state.year; }
  function filters() { return { year: yearFor(), branch: state.branch, area: state.area }; }
  ...
  function scopeLabel(m) {
    ...
    var yr = yearFor();
    parts.push(yr === "all" ? m.years[0] + "–" + m.years[m.years.length - 1] : yr);
    return parts.join(" · ");
  }
  function filterChips(m) {
    var selYear = yearFor();
    var yearOpts = '<option value="all"' + (selYear === "all" ? " selected" : "") + '>All years</option>' + m.years.map(function (y) { return '<option value="' + y + '"' + (selYear === y ? " selected" : "") + '>' + y + '</option>'; }).join("");
  ...
  function statusCard(c) {
    var inner = '<div class="num"' + (c.color ? ' style="color:' + c.color + '"' : "") + ">" + c.value + '</div><div class="sub" style="margin-top:2px">' + esc(c.label) + (c.tap ? ' <span style="color:#B9AEA8">›</span>' : "") + "</div>";
    return c.tap ? '<button class="card btncard" ' + c.tap + ">" + inner + "</button>" : '<div class="card">' + inner + "</div>";
  }
  ...
  function activeMonths(m) { var yr = yearFor(); return m.periods.map(function (p, pi) { return { p: p, pi: pi }; }).filter(function (x) { return x.pi <= m.lastActive && (yr === "all" || x.p.year === yr); }); }
  ...
  function renderHeatDetail() {
    ...
    var m = model(), f = filters();
    var defsBtn = function (attrs) { return m.kind === "defects" ? '<button class="chip" data-action="def-cell"' + attrs + ' style="margin-top:8px">View defects ›</button>' : ""; };
    if (isHeatAreaView(m) && sel.a != null) {
      el.innerHTML = '<div class="notebox" style="margin-bottom:10px"><div class="b t13">' + esc(m.branches[sel.b]) + " · " + esc(m.areas[sel.a]) + '</div><div class="row gap8 wrap" style="margin-top:6px">' + m.areaCellDetail(sel.b, sel.a, f) + "</div>" + defsBtn(' data-cb="' + sel.b + '" data-ca="' + sel.a + '"') + "</div>";
      return;
    }
    if (sel.p == null) { el.innerHTML = ""; return; }
    el.innerHTML = '<div class="notebox" style="margin-bottom:10px"><div class="b t13">' + esc(m.branches[sel.b]) + " · " + esc(m.periods[sel.p].full) + '</div><div class="row gap8 wrap" style="margin-top:6px">' + m.cellDetail(sel.b, sel.p, f) + "</div>" + defsBtn(' data-cb="' + sel.b + '" data-cp="' + sel.p + '"') + "</div>";
  }
  ...
  function renderRegister() {
    ...
    var rows = Object.keys(groups).map(function (k) { var rec = m.record(groups[k]); rec.name = groupBy === "area" ? m.areas[k] : m.branches[k]; rec.__gi = +k; rec.__gb = groupBy; return rec; });
    ...
    function cell(rec, c, isTotal) {
      var v = m.regField(rec, c.k);
      if (c.k === "defects" && m.kind === "defects" && (+v) > 0) {
        var attrs = isTotal ? "" : (rec.__gb === "area" ? ' data-ca="' + rec.__gi + '"' : ' data-cb="' + rec.__gi + '"');
        return '<td><button class="defnum" data-action="def-cell"' + attrs + ' style="background:none;border:none;padding:0;font:inherit;color:#8E0E1F;font-weight:600;text-decoration:underline;text-underline-offset:2px;cursor:pointer">' + fmtNum(v) + "</button></td>";
      }
    ...
    function rowHtml(rec, isTotal) { return "<tr" + (isTotal ? ' class="total"' : "") + ">" + m.regCols.map(function (c) { return cell(rec, c, isTotal); }).join("") + "</tr>"; }
    document.getElementById("content").innerHTML =
      filterChips(m) +
  ...
  // ---- PMAS-to-date block for the Operational Standard report only ----
  // (PMAS is derived from the operational score alone, so it belongs on the operational
  // report, not the branch defect report.) The operational report already renders the
  // month's Operational Standard verdict, KPIs and component table above, so this block
  // adds only the running PMAS figure.
  function opStdRatingOf(v) { return v == null ? null : (v >= 95 ? 5 : v >= 80 ? 4 : v >= 70 ? 3 : v >= 60 ? 2 : 1); }
  function pmasBlock(bi, iso) {
    var om = MODELS.opstd; if (!om || bi < 0) return "";
    var y = iso ? iso.slice(0, 4) : (state.year !== "all" ? state.year : om.years[om.years.length - 1]);
    var monthsY = []; om.periods.forEach(function (p, idx) { if (p.year === y) monthsY.push({ p: p, idx: idx }); });
    var target = null;
    if (iso) { for (var i = 0; i < monthsY.length; i++) if (monthsY[i].p.iso === iso) { target = monthsY[i]; break; } }
    if (!target) { for (var j = monthsY.length - 1; j >= 0; j--) { if (om.cellValue(bi, monthsY[j].idx, 0) != null) { target = monthsY[j]; break; } } }
    if (!target) return "";
    var mnum = parseInt(target.p.iso.slice(5, 7), 10);
    // PMAS to date — the worksheet method: sum of the year's monthly overalls up to this
    // month, divided by 6 (first half) or 12 (full year).
    var isMid = mnum <= 6, div = isMid ? 6 : 12, basis = isMid ? "Mid-year" : "Year-end";
    var vals = []; monthsY.forEach(function (x) { var mm = parseInt(x.p.iso.slice(5, 7), 10); if (mm <= mnum) { var v = om.cellValue(bi, x.idx, 0); if (v != null) vals.push(v); } });
    var sum = vals.reduce(function (a, b) { return a + b; }, 0), pmas = vals.length ? sum / div : null, pb = scoreBand(pmas);
    var verdict = function (val, rt, band) { return '<div class="rep-verdict"><span class="big" style="font-size:34px">' + (val == null ? "—" : fmtScore(val)) + '</span><span class="statpill" style="background:' + band.bg + ";color:" + band.c + '">' + (rt == null ? "—" : "Rating " + rt) + "</span></div>"; };
    return '<hr class="report-rule">' +
      '<div class="report-h">PMAS to date · ' + basis + " basis · " + esc(target.p.full) + "</div>" + verdict(pmas, opStdRatingOf(pmas), pb) +
      '<div class="sub13" style="margin-top:6px">Performance Management Assessment Score — the running average of the branch’s monthly Operational Standard score, ' + vals.length + " of " + div + " months in. The " + basis.toLowerCase() + " PMAS divides the year-to-date sum by " + div + " (mid-year at June, year-end at December).</div>";
  }

  // ---- Screen: Insights ----
  // Interpretive analysis (not a data dump): where to focus, found-vs-fixed gaps,
  // systemic recurring, and how operational scores relate to defects. Defect-centric,
  // scoped by the shared year filter, and cross-references the Operational Standard set.
  ...
  function renderInsights() {
    var m = MODELS.defects, o = MODELS.opstd, yr = state.year;
    var stats = m.branches.map(function (b, i) {
      var rec = m.record(m.filterRows({ year: yr, branch: i, area: -1 }));
      return { name: b, i: i, defects: rec.defects, instances: rec.instances, defectRate: rec.defectRate, resolutionRate: rec.resolutionRate,
        unresolved: (rec.resolvable || 0) - (rec.resolved || 0), recurRate: rec.defects ? rec.recurring / rec.defects : null };
    }).filter(function (x) { return x.instances > 0; });

    var yearOpts = '<option value="all"' + (yr === "all" ? " selected" : "") + '>All years</option>' + m.years.map(function (y) { return '<option value="' + y + '"' + (yr === y ? " selected" : "") + '>' + y + '</option>'; }).join("");
    var head = '<div class="row between mid" style="margin:2px 4px 10px;gap:8px"><div class="period-select-label">Insights · ' + esc(yr === "all" ? "all years" : String(yr)) + '</div><select class="sel" id="year-sel">' + yearOpts + '</select></div>';
    var content = document.getElementById("content");
    if (!stats.length) { content.innerHTML = head + '<div class="card"><div class="sub13">No defect data in this scope.</div></div>'; return; }

    // Card 1 — Branch watchlist (composite risk: defect rate, unresolved share, recurring).
    var maxDR = Math.max.apply(null, stats.map(function (s) { return s.defectRate || 0; })) || 1;
    var maxRR = Math.max.apply(null, stats.map(function (s) { return s.recurRate || 0; })) || 1;
    stats.forEach(function (s) { var unres = s.resolutionRate == null ? 0 : (1 - s.resolutionRate); s.risk = ((s.defectRate || 0) / maxDR) * 60 + unres * 25 + ((s.recurRate || 0) / maxRR) * 15; });
    var why = function (s) {
      var r = [];
      if ((s.defectRate || 0) >= maxDR * 0.7) r.push("high defect rate");
      if (s.resolutionRate != null && s.resolutionRate < 0.7) r.push("only " + fmtPct(s.resolutionRate) + " resolved");
      if ((s.recurRate || 0) >= 0.35) r.push(fmtPct(s.recurRate) + " recurring");
      return r.length ? r.join(" · ") : "elevated overall";
    };
    var watch = stats.slice().sort(function (a, b) { return b.risk - a.risk; }).slice(0, 5);
    var watchCard = '<div class="card" style="margin-bottom:12px;padding:4px 14px">' + watch.map(function (s, idx) {
      var b = defectBand(s.defectRate);
      return '<div style="padding:11px 0' + (idx ? ";border-top:1px solid #F4EFED" : "") + '"><div class="row between mid"><div style="min-width:0;flex:1;padding-right:8px"><div class="t14">' + (idx + 1) + ". " + esc(s.name) + '</div><div class="sub" style="margin-top:3px;color:#8A7E7A">' + esc(why(s)) + '</div></div><span class="statpill" style="background:' + b.bg + ";color:" + b.c + '">' + fmtPct(s.defectRate) + "</span></div></div>";
    }).join("") + "</div>";

    // Card 2 — Resolution gap (defects found vs actually fixed).
    var gap = stats.filter(function (s) { return s.defects >= 10 && s.resolutionRate != null; }).sort(function (a, b) { return a.resolutionRate - b.resolutionRate; }).slice(0, 4);
    var gapCard = gap.length && gap[0].resolutionRate < 0.95 ? '<div class="card" style="margin-bottom:12px;padding:4px 14px">' + gap.map(function (s, idx) {
      return '<div class="row between mid" style="padding:10px 0' + (idx ? ";border-top:1px solid #F4EFED" : "") + '"><div style="min-width:0;flex:1;padding-right:8px"><div class="t13">' + esc(s.name) + '</div><div class="sub" style="margin-top:2px;color:#8A7E7A">' + fmtNum(s.defects) + " defects · " + fmtNum(s.unresolved) + ' still open</div></div><span class="statpill" style="background:' + (s.resolutionRate < 0.6 ? "#FBE3E7;color:#8E0E1F" : "#FBF2DC;color:#A07208") + '">' + fmtPct(s.resolutionRate) + " resolved</span></div>";
    }).join("") + "</div>" : '<div class="card" style="margin-bottom:12px"><div class="sub13">Resolution is healthy across branches in this scope.</div></div>';

    // Card 3 — Systemic recurring (process areas where defects keep coming back).
    var areaStats = m.areas.map(function (a, ai) {
      var rec = m.record(m.filterRows({ year: yr, branch: -1, area: ai }));
      var topB = null, topRec = -1;
      m.branches.forEach(function (b, bi) { var rr = m.record(m.filterRows({ year: yr, branch: bi, area: ai })); if (rr.recurring > topRec) { topRec = rr.recurring; topB = b; } });
      return { area: a, defects: rec.defects, recurRate: rec.defects ? rec.recurring / rec.defects : null, topBranch: topB };
    }).filter(function (x) { return x.defects >= 20 && x.recurRate != null; }).sort(function (a, b) { return b.recurRate - a.recurRate; }).slice(0, 4);
    var recCard = areaStats.length ? '<div class="card" style="margin-bottom:12px;padding:4px 14px">' + areaStats.map(function (s, idx) {
      return '<div class="row between mid" style="padding:10px 0' + (idx ? ";border-top:1px solid #F4EFED" : "") + '"><div style="min-width:0;flex:1;padding-right:8px"><div class="t13">' + esc(s.area) + '</div><div class="sub" style="margin-top:2px;color:#8A7E7A">' + fmtNum(s.defects) + " defects · most at " + esc(s.topBranch || "—") + '</div></div><span class="statpill" style="background:#FBF2DC;color:#C75F00">' + fmtPct(s.recurRate) + " recur</span></div>";
    }).join("") + "</div>" : "";

    // Card 4 — Operational standard vs defects (does one predict the other?).
    var examples = [];
    stats.forEach(function (s) {
      var oi = o.branches.indexOf(s.name); if (oi < 0 || s.defectRate == null) return;
      var op = o.aggregate(o.filterRows({ year: yr, branch: oi, area: -1 }), o.headline.key);
      if (op == null) return; examples.push({ name: s.name, op: op, dr: s.defectRate });
    });
    var r = pearson(examples.map(function (e) { return [e.op, e.dr]; }));
    var corrCard;
    if (r == null) { corrCard = '<div class="card" style="margin-bottom:12px"><div class="sub13">Not enough overlapping data to compare operational scores with defect rates in this scope.</div></div>'; }
    else {
      var strength = Math.abs(r) < 0.3 ? "little to no" : Math.abs(r) < 0.6 ? "a moderate" : "a strong";
      var msg = Math.abs(r) < 0.3
        ? "There's <b>" + strength + "</b> relationship between operational-standard scores and defect rates across branches (r = " + r.toFixed(2) + ") — they move largely independently."
        : "There's <b>" + strength + "</b> relationship (r = " + r.toFixed(2) + "): " + (r < 0 ? "lower operational scores tend to come with higher defect rates" : "higher operational scores oddly come with higher defect rates — worth a look") + ".";
      var worstOp = examples.slice().sort(function (a, b) { return a.op - b.op; })[0];
      var worstDr = examples.slice().sort(function (a, b) { return b.dr - a.dr; })[0];
      corrCard = '<div class="card" style="margin-bottom:12px"><div class="sub13" style="line-height:1.6">' + msg + "</div>" +
        '<div class="row gap8" style="margin-top:10px"><div style="flex:1"><div class="period-select-label">Lowest op score</div><div class="t13" style="margin-top:3px">' + esc(worstOp.name) + " · " + fmtScore(worstOp.op) + " · " + fmtPct(worstOp.dr) + ' defects</div></div>' +
        '<div style="flex:1"><div class="period-select-label">Highest defect rate</div><div class="t13" style="margin-top:3px">' + esc(worstDr.name) + " · " + fmtPct(worstDr.dr) + " · op " + fmtScore(worstDr.op) + "</div></div></div></div>";
    }

    content.innerHTML = head +
      '<div class="h2l" style="margin-top:0">Branch watchlist · where to focus</div><div class="sub13" style="margin:0 4px 6px">Ranked by combined defect rate, unresolved share and recurring.</div>' + watchCard +
      '<div class="h2l">Resolution gap · found vs fixed</div>' + gapCard +
      (recCard ? '<div class="h2l">Systemic recurring · repeat defects</div>' + recCard : "") +
      '<div class="h2l">Operational standard vs defects</div>' + corrCard;
  }
  ...
  function renderReport() {
    var m = model();
    var f = filters();
    // The report is monthly (branch defects and operational standard alike): default to
    // the latest month in scope, with a "Whole year" option.
    ...
    var monthPicker = '<div class="period-select" style="margin:2px 4px 10px"><span class="period-select-label">Report month</span><select class="period-select-input" id="rep-month">' +
    ...
    document.getElementById("content").innerHTML =
      ...
      '<div class="report">' +
        '<div class="rep-head">' +
          '<div class="rep-eyebrow">Pulsus Quality · Operational Risk &amp; Audit</div>' +
          '<div class="report-title">' + esc(repTitle) + "</div>" +
          '<div class="sub13" style="margin-top:6px">' + esc(repScope) + "</div>" +
          '<div class="sub" style="margin-top:3px">Generated ' + esc(genDate) + "</div>" +
        "</div>" +
        ...
        (m.kind === "opstd" && state.branch >= 0 ? pmasBlock(state.branch, repPi >= 0 ? m.periods[repPi].iso : null) : "") +
      "</div>" + footer(m);
  }
  ...
  // Re-fetch submissions in the background (keeps the current list visible, no flicker) so
  // an open Review/Mine queue reflects submissions made since it was opened.
  function reloadSubs() {
    if (state.subsLoading) return;
    state.subsLoading = true;
    api.listSubmissions().then(function (r) {
      state.subsLoading = false;
      if (r.ok) { state.subs = r.submissions; state.subsErr = ""; }
      else if (state.subs === null) { state.subs = []; state.subsErr = r.error; }
      if (["entry", "mine", "review", "report"].indexOf(state.tab) >= 0) render();
    });
  }
  ...
  // Standard defect natures per process area (from the branch reviews + the Nature of
  // Defects standards), so the Defect Area is picked from a controlled list — "Other…"
  // still allows a free-text exception.
  var COMMON_NATURES = ["Form completion", "ID capture", "Signature verification", "CIF records"];
  var NATURE_BY_AREA = {
    "ABM: Card Application & Amendment": ["CMS keying", "ID capture", "Card issuance", "Indemnity form", "Signature verification", "Form completion", "CIF records"],
    "Addition of Name": ["CIF records", "Status 3", "Mandate / signature", "Form completion", "ID capture"],
    "CIF Merge/Deleting Customer Records": ["CIF merge approval", "Surviving record", "ID capture", "Form completion"],
    "Standing Order: New & Amendment": ["SO setup", "Start / end date", "Signature verification", "Form completion", "ID capture"],
    "Wire Transfers": ["Form completion", "Endorsement", "Beneficiary details", "CIF records", "Signature verification"],
  };
  function naturesFor(area) { return NATURE_BY_AREA[area] || COMMON_NATURES; }
  function blankRow() { var c = {}; CHECK_LABELS.forEach(function (x) { c[x[0]] = "yes"; }); return { area: "", txnType: "Application", checks: c, defects: 0, defectArea: "", natureMode: "list", recurring: false, status: "open" }; }

  // Operational Standard — officers key the raw measures; the app computes the tree
  // (Major Procedure, Risk Metrics, Overall %, 1–5 rating), mirroring the server.
  var OPSTD_SOP = [["screening", "Customer screening"], ["wires", "Wire transfer processing"], ["sourceOfFunds", "Source of funds"], ["csl", "Cash-secured loan delinquency"], ["fees", "Fee collection"], ["declaration", "Branch declaration"], ["salesService", "Sales & service routine"], ["acknowledgement", "Team-member acknowledgement"]];
  var OPSTD_TOP = [["complaints", "Customer complaints"], ["procurement", "Procurement"], ["queueSla", "Queue SLA (wait time)"], ["onboardingSla", "Onboarding SLA"], ["auditResolution", "Audit resolution"]];
  var AUDIT_LETTERS = [["", "—"], ["A", "A (100)"], ["B+", "B+ (80)"], ["B", "B (60)"], ["C", "C (40)"], ["D", "D (20)"]];
  var AUDIT_POINTS = { A: 100, "B+": 80, B: 60, C: 40, D: 20 };
  function computeOpStd(values, auditPoints) {
    function n(k) { var x = values[k]; return x === "" || x == null ? null : Number(x); }
    ...
    var major = mean(OPSTD_SOP.map(function (s) { return n(s[0]); }));
    if (major != null) major = Math.round(major); // whole number, matching the worksheet
    var risk = mean([n("onboardingSla"), auditPoints, n("auditResolution")]);
    var overall = mean([n("complaints"), n("procurement"), risk, n("queueSla"), major]);
    var avgSla = mean([n("queueSla"), n("onboardingSla")]);
    var rating = overall == null ? null : (overall >= 95 ? 5 : overall >= 80 ? 4 : overall >= 70 ? 3 : overall >= 60 ? 2 : 1);
    return { major: major, risk: risk, overall: overall, avgSla: avgSla, rating: rating };
  }
  ...
  function ensureEntry() {
    ...
    var today = todayISO();
    state.entry = { id: null, dataset: ds, branch: (session && session.branch) || "", period: periodOf(today), entryDate: today, attachments: [], items: [], row: blankRow(), values: {}, audit: "", msg: "", msgOk: false };
  }
  // Today's local calendar date as yyyy-mm-dd, for the entry-date default.
  function todayISO() { var d = new Date(); return d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, "0") + "-" + String(d.getDate()).padStart(2, "0"); }
  // Format a yyyy-mm-dd string as "5 Aug 2026" without timezone drift (parse the parts, not the string).
  function fmtDate(iso) { var mo = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]; var p = /^(\d{4})-(\d{2})-(\d{2})$/.exec(iso || ""); return p ? +p[3] + " " + mo[+p[2] - 1] + " " + p[1] : (iso || "—"); }
  // The reporting month a picked date falls in, e.g. "Aug 2026".
  function monthLabel(iso) { var mo = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]; var p = /^(\d{4})-(\d{2})/.exec(iso || ""); return p ? mo[+p[2] - 1] + " " + p[1] : "—"; }
  // The first-of-month ISO (yyyy-mm-01) a picked date maps to — the dataset's period key.
  function periodOf(iso) { var p = /^(\d{4}-\d{2})/.exec(iso || ""); return p ? p[1] + "-01" : ""; }
  ...
  function readEntryForm() {
    ...
    // A single date now drives the entry: its month is the reporting period.
    if ((v = g("entry-date")) != null && v !== "") { e.entryDate = v; e.period = periodOf(v); }
    if (e.dataset === "opstd") { OPSTD_SOP.concat(OPSTD_TOP).forEach(function (s) { var x = g("entry-" + s[0]); if (x != null) e.values[s[0]] = x; }); var au = g("entry-audit"); if (au != null) e.audit = au; }
  }
  function readEntryRow() {
    ...
    if ((v = g("er-defects")) != null) { var d = parseInt(v, 10); r.defects = isNaN(d) || d < 0 ? 0 : d; }
    CHECK_LABELS.forEach(function (c) { var x = g("er-chk-" + c[0]); if (x != null) r.checks[c[0]] = x; });
    var nsel = g("er-defectarea-sel");
    if (nsel != null) {
      if (nsel === "__other") { r.natureMode = "other"; var ot = g("er-defectarea-other"); if (ot != null) r.defectArea = ("" + ot).trim(); }
      else { r.natureMode = "list"; r.defectArea = nsel; }
    }
  ...
  function entryPreviewHtml(e) {
    if (e.dataset === "opstd") {
      var pts = AUDIT_POINTS[e.audit] != null ? AUDIT_POINTS[e.audit] : null;
      var c = computeOpStd(e.values, pts), fs = function (x) { return x == null ? "—" : fmtScore(x); };
      var otile = function (val, lbl, color) { return '<div style="flex:1;min-width:74px;text-align:center"><div class="num18"' + (color ? ' style="color:' + color + '"' : "") + ">" + val + '</div><div class="sub" style="margin-top:2px">' + lbl + "</div></div>"; };
      return '<div class="row wrap gap10" style="justify-content:space-between">' +
        otile(fs(c.major), "Major procedure") + otile(fs(c.risk), "Risk metrics") + otile(fs(c.overall), "Overall %", scoreBand(c.overall).c) +
        otile(c.rating == null ? "—" : c.rating, "Rating 1–5", c.overall == null ? null : scoreBand(c.overall).c) + otile(fs(c.avgSla), "Avg customer SLA") + "</div>" +
        '<div class="notebox" style="margin-top:10px"><b>Overall</b> = average of Complaints, Procurement, Risk Metrics, Queue SLA &amp; Major Procedure. <b>Major Procedure</b> = average of the 8 SOP standards. <b>Risk Metrics</b> = average of Onboarding SLA, Audit points &amp; Audit Resolution. All computed — leave a measure blank if not assessed.</div>';
    }
  ...
  function rowFormHtml(e) {
    ...
    var sel = function (id, opts, cur, extra) { return '<select class="sel" id="' + id + '" style="width:100%' + (extra || "") + '">' + opts.map(function (o) { var val = Array.isArray(o) ? o[0] : o, lbl = Array.isArray(o) ? o[1] : o; return '<option value="' + esc(val) + '"' + (cur === val ? " selected" : "") + ">" + esc(lbl) + "</option>"; }).join("") + "</select>"; };
    ...
    var natures = naturesFor(r.area);
    var isOther = r.natureMode === "other" || (r.defectArea && natures.indexOf(r.defectArea) < 0);
    var natOpts = [["", "Choose nature…"]].concat(natures.map(function (n) { return [n, n]; })).concat([["__other", "Other…"]]);
    var defectFields = r.defects > 0
      ? '<div style="margin-top:10px"><div>' + lbl("Defect area / nature") + sel("er-defectarea-sel", natOpts, isOther ? "__other" : (r.defectArea || "")) +
        (isOther ? '<input class="login-in" id="er-defectarea-other" style="margin:8px 0 0" placeholder="Describe the defect" value="' + esc(r.defectArea || "") + '">' : "") + "</div>" +
    ...
    return '<div class="card" style="margin-bottom:12px">' +
      ...
      '<div style="margin-top:12px;max-width:220px">' + lbl("Number of defects on this transaction") +
        '<input class="login-in" id="er-defects" inputmode="numeric" autocomplete="off" style="margin-bottom:0" value="' + (r.defects || 0) + '" placeholder="0"></div>' +
      defectFields +
  ...
  function renderEntry() {
    ...
    // Branch is always a dropdown (defaulting to the officer's own branch) so it can be changed.
    var branchSel = '<select class="sel" id="entry-branch" style="width:100%">' + (e.branch ? "" : '<option value="">Choose branch…</option>') + branches.map(function (b) { return '<option value="' + esc(b) + '"' + (e.branch === b ? " selected" : "") + ">" + esc(b) + "</option>"; }).join("") + "</select>";
    var dateSel = '<input type="date" class="sel" id="entry-date" style="width:100%" value="' + esc(e.entryDate || todayISO()) + '">';
    var header = '<div class="card" style="margin-bottom:12px">' +
      ...
      '<div class="row gap8"><div style="flex:1;min-width:0"><div class="period-select-label" style="margin-bottom:5px">Branch</div>' + branchSel + "</div>" +
        '<div style="flex:1;min-width:0"><div class="period-select-label" style="margin-bottom:5px">Date</div>' + dateSel +
          '<div class="sub13" style="margin-top:4px;color:#8A7E7A">Reporting month · ' + esc(monthLabel(e.entryDate || todayISO())) + "</div></div></div></div>";
    var body;
    if (e.dataset === "opstd") {
      var auditSel = '<div><div class="period-select-label" style="margin-bottom:5px">Audit grade</div><select class="sel" id="entry-audit" style="width:100%">' +
        AUDIT_LETTERS.map(function (l) { return '<option value="' + l[0] + '"' + (e.audit === l[0] ? " selected" : "") + ">" + esc(l[1]) + "</option>"; }).join("") + "</select></div>";
      body = '<div class="h2l">SOP adherence · 0–100</div><div class="card" style="margin-bottom:12px"><div class="grid-status" style="grid-template-columns:1fr 1fr;margin-top:0">' +
        OPSTD_SOP.map(function (s) { return numField("entry-" + s[0], s[1], e.values[s[0]]); }).join("") + "</div></div>" +
        '<div class="h2l">Other measures · 0–100</div><div class="card" style="margin-bottom:12px"><div class="grid-status" style="grid-template-columns:1fr 1fr;margin-top:0">' +
        OPSTD_TOP.map(function (s) { return numField("entry-" + s[0], s[1], e.values[s[0]]); }).join("") + auditSel + "</div></div>" +
    ...
    // Audit sample-findings attachment — available on both entry types.
    var atts = e.attachments || [];
    var attList = atts.length
      ? '<div style="margin-bottom:10px">' + atts.map(function (a) {
          return '<div class="row between mid" style="padding:8px 0;border-top:1px solid #F4EFED"><a href="/api/attachment?id=' + encodeURIComponent(a.id) + '" target="_blank" rel="noopener" class="t13" style="min-width:0;flex:1;padding-right:8px;color:var(--ink);text-decoration:none">📎 ' + esc(a.name) + ' <span class="sub" style="color:#8A7E7A">' + esc(fmtBytes(a.size)) + "</span></a>" +
            '<button class="modalx" data-action="entry-delattach" data-id="' + esc(a.id) + '">✕</button></div>';
        }).join("") + "</div>"
      : '<div class="sub13" style="margin-bottom:10px;color:#8A7E7A">No attachment yet.</div>';
    var attachCard = '<div class="h2l">Audit sample findings</div>' +
      '<div class="card" style="margin-bottom:12px">' +
        '<div class="sub13" style="margin-bottom:10px">Attach the sample findings from the audit (PDF, image, spreadsheet or document).</div>' +
        attList +
        '<label class="cta2" style="margin:0;cursor:pointer;text-align:center;display:block">⬆ Upload attachment<input type="file" id="entry-attach" style="display:none"></label></div>';
    ...
    document.getElementById("content").innerHTML =
      ...
      header + body + attachCard + msgHtml +
  ...
  function subRowHtml(s, kind) {
    var meta = datasetLabel(s.dataset) + " · " + periodFull(s.dataset, s.period) + (s.entryDate ? " · Entered " + fmtDate(s.entryDate) : "");
  ...
  // Audit sample-findings attached to a submission, as downloadable links.
  function attachmentsHtml(s) {
    var atts = s.attachments || [];
    if (!atts.length) return "";
    return '<div class="h2l">Audit sample findings</div><div class="card" style="margin-bottom:10px;padding:4px 14px">' +
      atts.map(function (a) {
        return '<div class="row between mid" style="padding:9px 0;border-top:1px solid #F4EFED"><a href="/api/attachment?id=' + encodeURIComponent(a.id) + '" target="_blank" rel="noopener" class="t13" style="min-width:0;flex:1;padding-right:8px;color:var(--ink);text-decoration:none">📎 ' + esc(a.name) + '</a><span class="sub" style="color:#8A7E7A">' + esc(fmtBytes(a.size)) + "</span></div>";
      }).join("") + "</div>";
  }
  ...
  function showSubModal(id) {
    ...
    document.getElementById("modal-root").innerHTML =
      '<div class="overlay" data-action="modal-close"><div class="modal" data-stop style="max-width:380px;max-height:86vh;overflow-y:auto">' +
        ...
        '<div class="sub13" style="margin:4px 0 14px">' + esc(datasetLabel(s.dataset) + " · " + periodFull(s.dataset, s.period)) + " · from <b>" + esc(s.inspector) + "</b> " + statusPill(s.status) + (s.entryDate ? '<br><span style="color:#8A7E7A">Date of entry &middot; ' + esc(fmtDate(s.entryDate)) + "</span>" : "") + "</div>" +
        '<div class="h2l" style="margin-top:0">Values submitted</div><div style="padding:0 2px 4px">' + valuesTableHtml(s) + "</div>" +
        (s.dataset === "defects" && (s.items || []).some(function (it) { return (it.defects || 0) > 0; }) ? '<button class="cta2" data-action="def-sub" data-id="' + s.id + '" style="margin:2px 0 12px">View &amp; resolve defects ›</button>' : "") +
        attachmentsHtml(s) +
        '<div class="h2l">Audit trail</div><div style="padding:0 2px">' + auditTrailHtml(s) + "</div></div></div>";
  }

  // ---- Defect drill-down: the individual defects behind a count, with resolution ----
  function canResolve() { return role() === "supervisor" || role() === "admin"; }
  function defKey(it) { return it.subId + "#" + it.index; }
  function parseDefKey(k) { var i = k.lastIndexOf("#"); return { id: k.slice(0, i), index: +k.slice(i + 1) }; }
  // The first-of-month ISOs for a year filter ("all" → every period), used to scope a drill-down.
  function periodsForYear(m, yr) { return m.periods.filter(function (p) { return yr === "all" || p.year === yr; }).map(function (p) { return p.iso; }); }
  // Build a drill-down scope from a dataset cell (branch/area indexes, optional period index).
  function scopeFromCell(bi, ai, pi) {
    var m = MODELS.defects;
    var branch = bi != null && bi >= 0 ? m.branches[bi] : null;
    var area = ai != null && ai >= 0 ? m.areas[ai] : null;
    var periods = pi != null && pi >= 0 ? [m.periods[pi].iso] : periodsForYear(m, state.year);
    var when = pi != null && pi >= 0 ? m.periods[pi].full : (state.year === "all" ? "all periods" : state.year);
    return { branch: branch, area: area, periods: periods, title: (branch || "All branches") + (area ? " · " + area : "") + " · " + when };
  }
  // Gather individual defect line items matching a scope. scope = { subId } | { branch, area, periods }.
  function collectDefectItems(scope) {
    var subs = (state.subs || []).filter(function (s) { return s.dataset === "defects"; });
    if (scope.subId) subs = subs.filter(function (s) { return s.id === scope.subId; });
    else {
      subs = subs.filter(function (s) { return s.status === "published"; });
      if (scope.branch) subs = subs.filter(function (s) { return s.branch === scope.branch; });
      if (scope.periods) subs = subs.filter(function (s) { return scope.periods.indexOf(s.period) >= 0; });
    }
    var out = [];
    subs.forEach(function (s) {
      (s.items || []).forEach(function (it, i) {
        if ((it.defects || 0) <= 0) return;
        if (scope.area && it.area !== scope.area) return;
        out.push({ subId: s.id, index: i, area: it.area, txnType: it.txnType, defectArea: it.defectArea, recurring: it.recurring, defects: it.defects, status: it.status, subStatus: s.status, period: s.period });
      });
    });
    return out;
  }
  function defOverlay(inner) {
    return '<div class="overlay" data-action="modal-close"><div class="modal" data-stop style="max-width:400px;max-height:86vh;overflow-y:auto">' +
      '<button class="modalx" data-action="modal-close">✕</button>' + inner + "</div></div>";
  }
  function openDefects(scope) {
    state.defModal = { scope: scope, sel: {}, msg: "" };
    if (state.subs === null) {
      document.getElementById("modal-root").innerHTML = defOverlay('<div class="sub13" style="padding:22px 0;text-align:center;color:#8A7E7A">Loading defects…</div>');
      if (!state.subsLoading) {
        state.subsLoading = true;
        api.listSubmissions().then(function (r) {
          state.subsLoading = false;
          if (r.ok) { state.subs = r.submissions; state.subsErr = ""; } else { state.subs = []; state.subsErr = r.error; }
          if (state.defModal) renderDefectsModal();
        });
      }
    } else renderDefectsModal();
  }
  function renderDefectsModal() {
    var dm = state.defModal; if (!dm) return;
    var items = collectDefectItems(dm.scope);
    var total = items.length, resolved = items.filter(function (x) { return x.status === "resolved"; }).length;
    var mayResolve = canResolve();
    var head = '<div class="t15" style="padding-right:22px">Defects · ' + esc(dm.scope.title || "") + "</div>" +
      '<div class="sub13" style="margin:4px 0 12px">' + total + " defect" + (total === 1 ? "" : "s") + " · " + resolved + " resolved · " + (total - resolved) + " open</div>";
    if (dm.msg) head += '<div class="msgbox ok" style="margin-bottom:10px">' + esc(dm.msg) + "</div>";
    if (!total) {
      document.getElementById("modal-root").innerHTML = defOverlay(head + '<div class="card"><div class="sub13">No individual defect detail behind this count. Only reviews keyed in the app carry line-item defects — bundled historical figures are counts only.</div></div>');
      return;
    }
    var selKeys = Object.keys(dm.sel).filter(function (k) { return dm.sel[k]; });
    var bulk = "";
    if (mayResolve) {
      var openPub = items.filter(function (x) { return x.subStatus === "published" && x.status !== "resolved"; });
      var btns = selKeys.length
        ? '<button class="cta2" data-action="def-mark" data-res="1" style="flex:1;margin:0">Mark ' + selKeys.length + ' resolved</button><button class="cta2" data-action="def-mark" data-res="0" style="flex:1;margin:0;color:#8A7E7A">Reopen ' + selKeys.length + "</button>"
        : '<div class="sub13" style="color:#8A7E7A">Tick defects to resolve as a group, or use the ✓/↺ on any row.</div>';
      bulk = '<div class="row gap8 mid" style="margin:0 0 10px">' + btns + "</div>" +
        (openPub.length ? '<div class="row gap8" style="margin-bottom:10px"><button class="chip" data-action="def-all">Select all open (' + openPub.length + ")</button>" + (selKeys.length ? '<button class="chip" data-action="def-clr">Clear</button>' : "") + "</div>" : "");
    }
    var list = items.map(function (it) {
      var k = defKey(it), isRes = it.status === "resolved", canRow = mayResolve && it.subStatus === "published";
      var pill = isRes ? '<span class="statpill" style="background:#E5F4EC;color:#0E8A4D">Resolved</span>' : '<span class="statpill" style="background:#FBE3E7;color:#8E0E1F">Open</span>';
      var check = canRow ? '<button class="defcheck" data-action="def-toggle" data-k="' + esc(k) + '" style="flex-shrink:0;width:22px;height:22px;border-radius:6px;font-size:12px;line-height:1;border:1.5px solid ' + (dm.sel[k] ? "#1C1416;background:#1C1416;color:#fff" : "#E4DDD9;background:#fff;color:#fff") + '">' + (dm.sel[k] ? "✓" : "·") + "</button>" : "";
      var rowAct = canRow ? '<button class="chip" data-action="def-one" data-k="' + esc(k) + '" data-res="' + (isRes ? "0" : "1") + '">' + (isRes ? "↺" : "✓") + "</button>" : "";
      var det = esc(it.area) + (it.txnType ? " · " + esc(it.txnType) : "") + (it.defectArea ? " · " + esc(it.defectArea) : "") + (it.recurring ? " · recurring" : "");
      return '<div class="row between mid" style="padding:10px 0;border-top:1px solid #F4EFED"><div class="row mid gap8" style="min-width:0;flex:1;padding-right:8px">' + check + '<div style="min-width:0"><div class="t13">' + det + '</div><div class="sub" style="margin-top:2px">' + esc(periodFull("defects", it.period)) + " · " + it.defects + " defect" + (it.defects === 1 ? "" : "s") + "</div></div></div><div class=\"row mid gap8\" style=\"flex-shrink:0\">" + pill + rowAct + "</div></div>";
    }).join("");
    var note = mayResolve ? "" : '<div class="sub13" style="margin-top:10px;color:#8A7E7A">Supervisors and admins can mark defects resolved.</div>';
    document.getElementById("modal-root").innerHTML = defOverlay(head + bulk + '<div class="card" style="padding:2px 14px">' + list + "</div>" + note);
  }
  async function doResolve(refs, resolved) {
    var dm = state.defModal; if (!dm || !refs.length) return;
    var res = await api.resolveDefects(refs, resolved);
    if (!res.ok) { document.getElementById("modal-root").innerHTML = defOverlay('<div class="msgbox info">' + esc(res.error) + '</div><button class="cta2" data-action="modal-close" style="margin-top:10px">Close</button>'); return; }
    state.subs = res.submissions;
    await api.refreshData();      // pull the re-aggregated dataset so the resolution rate updates
    dm.sel = {};
    dm.msg = "✓ " + (resolved ? "Resolved " : "Reopened ") + res.changed + " defect" + (res.changed === 1 ? "" : "s") + ".";
    render();                     // refresh the screen behind the modal (leaves #modal-root intact)
    renderDefectsModal();
  }
  ...
  function renderReviewDetail() {
    ...
    content.innerHTML =
      ...
      (s.dataset === "defects" && (s.items || []).some(function (it) { return (it.defects || 0) > 0; }) ? '<button class="cta2" data-action="def-sub" data-id="' + s.id + '" style="margin-bottom:12px">View &amp; resolve defects ›</button>' : "") +
      attachmentsHtml(s) +
  ...
  var LOGIN_MARK = '<img src="/pulsus-quality.png" alt="Pulsus Quality" style="width:82%;max-width:300px;height:auto;display:block;margin:0 auto">';
  function showLoginModal() {
    ...
    if (L.mode === "admin") {
      ...
      var msSvg = '<svg width="18" height="18" viewBox="0 0 21 21" aria-hidden style="flex:none"><rect x="1" y="1" width="9" height="9" fill="#F25022"/><rect x="11" y="1" width="9" height="9" fill="#7FBA00"/><rect x="1" y="11" width="9" height="9" fill="#00A4EF"/><rect x="11" y="11" width="9" height="9" fill="#FFB900"/></svg>';
      var msDivider = '<div class="row mid" style="gap:10px;margin:14px 0 2px"><span style="flex:1;height:1px;background:#EEE7E3"></span><span class="sub" style="font-size:12px">or sign in by role</span><span style="flex:1;height:1px;background:#EEE7E3"></span></div>';
      // Live when SSO is configured; otherwise a disabled "· soon" placeholder so
      // the option is visibly there.
      var msBtn = (SSO_ENABLED
        ? ('<button class="login-in" data-action="sso-login" style="margin:16px 0 0;display:flex;align-items:center;justify-content:center;gap:10px;cursor:pointer;font-weight:600;background:#fff">' + msSvg + 'Sign in with Microsoft</button>')
        : ('<button class="login-in" disabled title="Coming soon — awaiting Microsoft Entra ID setup" style="margin:16px 0 0;display:flex;align-items:center;justify-content:center;gap:10px;cursor:not-allowed;opacity:.6;font-weight:600;background:#fff">' + msSvg + 'Sign in with Microsoft <span class="sub" style="font-weight:600">· soon</span></button>')
      ) + msDivider;
      inner = header + msBtn +
  ...
  var TABS = [
    ...
    { id: "insights", label: "Insights", icon: "M9 18h6M10 22h4M12 2a7 7 0 0 0-4 12.7c.6.5 1 1.3 1 2.1V17h6v-.2c0-.8.4-1.6 1-2.1A7 7 0 0 0 12 2z" },
  ...
  var NAV = {
    guest: ["pulse", "register", "heatmap", "insights", "report", "settings"],
    inspector: ["entry", "mine", "pulse", "heatmap", "insights", "report"],
    supervisor: ["review", "pulse", "register", "heatmap", "insights", "report"],
    admin: ["pulse", "register", "heatmap", "insights", "review", "entry", "settings"],
  };
  ...
  function render() {
    ...
    // The dataset toggle only drives the dashboards; on the entry/review screens the
    // dataset is chosen in the form, so hide the toggle there to avoid a second control.
    var mtw = document.getElementById("mode-toggle-wrap");
    if (mtw) mtw.style.display = ["pulse", "register", "heatmap", "report", "settings"].indexOf(state.tab) >= 0 ? "" : "none";
    renderTabs();
  ...
  function reportHTML(m, rows, from, to) {
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><title>' + esc(repTitle) + '</title><style>' +
      ...
      '<div class="top"><div class="eye">Pulsus Quality · Operational Risk &amp; Audit</div><h1>' + esc(repTitle) + "</h1>" +
      '<div class="meta">' + esc(scope) + " · " + esc(rangeLabel) + "<br>Generated " + esc(genDate) + "</div></div>" +
  ...
  async function handleAction(a, el) {
    ...
    if (a === "sso-login") { window.location.href = "/api/auth/microsoft/start"; return; }
    ...
    if (a === "modal-close") { hideModal(); state.pendingTab = null; state.defModal = null; return; }
    if (a === "def-cell") { if (state.mode === "defects") { var dcb = el.getAttribute("data-cb"), dca = el.getAttribute("data-ca"), dcp = el.getAttribute("data-cp"); openDefects(scopeFromCell(dcb != null ? +dcb : state.branch, dca != null ? +dca : state.area, dcp != null ? +dcp : -1)); } return; }
    if (a === "def-sub") { var dsub = null; (state.subs || []).forEach(function (x) { if (x.id === arg) dsub = x; }); openDefects({ subId: arg, title: dsub ? dsub.branch + " · " + periodFull("defects", dsub.period) : "review" }); return; }
    if (a === "def-toggle") { if (state.defModal) { var dk = el.getAttribute("data-k"); state.defModal.sel[dk] = !state.defModal.sel[dk]; renderDefectsModal(); } return; }
    if (a === "def-all") { if (state.defModal) { collectDefectItems(state.defModal.scope).forEach(function (it) { if (it.subStatus === "published" && it.status !== "resolved") state.defModal.sel[defKey(it)] = true; }); renderDefectsModal(); } return; }
    if (a === "def-clr") { if (state.defModal) { state.defModal.sel = {}; renderDefectsModal(); } return; }
    if (a === "def-one") { doResolve([parseDefKey(el.getAttribute("data-k"))], el.getAttribute("data-res") === "1"); return; }
    if (a === "def-mark") { if (state.defModal) { doResolve(Object.keys(state.defModal.sel).filter(function (k) { return state.defModal.sel[k]; }).map(parseDefKey), el.getAttribute("data-res") === "1"); } return; }
    ...
    if (a === "entry-delattach") { var ea = state.entry; if (ea && ea.attachments) ea.attachments = ea.attachments.filter(function (x) { return x.id !== arg; }); renderEntry(); return; }
    if (a === "entry-save" || a === "entry-submit") {
      readEntryForm(); var e = state.entry;
      var body = { action: a === "entry-submit" ? "submit" : "save", id: e.id || undefined, dataset: e.dataset, branch: e.branch, period: e.period, entryDate: e.entryDate, attachments: e.attachments || [] };
    ...
    if (a === "edit-sub") {
      ...
      if (!sub) return;
      state.entry = { id: sub.id, dataset: sub.dataset, branch: sub.branch, period: sub.period, entryDate: sub.entryDate || (sub.period || todayISO()), attachments: (sub.attachments || []).map(function (a) { return assign({}, a); }), area: sub.area || "", items: (sub.items || []).map(function (it) { return assign({}, it); }), row: blankRow(), values: assign({}, sub.values), audit: sub.audit == null ? "" : sub.audit, msg: sub.status === "returned" && sub.note ? "Returned: " + sub.note : "", msgOk: false };
  ...
  // Upload a sample-findings attachment; the bytes are stored server-side and referenced on the submission.
  async function handleAttachUpload(file) {
    ensureEntry(); var e = state.entry; readEntryForm();
    if (file.size > 15 * 1048576) { e.msg = "Attachment is too large (max 15 MB)."; e.msgOk = false; renderEntry(); return; }
    e.attachments = e.attachments || [];
    e.msg = "Uploading " + file.name + "…"; e.msgOk = true; renderEntry();
    try {
      var res = await api.uploadAttachment(file);
      if (!res.ok) { e.msg = res.error || "Upload failed."; e.msgOk = false; renderEntry(); return; }
      e.attachments.push(res.attachment);
      e.msg = "✓ Attached " + res.attachment.name + "."; e.msgOk = true; renderEntry();
    } catch (x) { e.msg = "Upload failed."; e.msgOk = false; renderEntry(); }
  }

  // ---- Events ----
  function onClick(e) {
    var t = e.target.closest("[data-mode],[data-tab],[data-branch],[data-area],[data-heat],[data-b][data-p],[data-ba][data-aa],[data-sort],[data-action],[data-dlfmt],[data-revds],[data-revbranch],[data-revsort],[data-audit],[data-stop]");
    if (!t) return;
    ...
    if (t.hasAttribute("data-tab")) {
      ...
      state.tab = tab; state.heatSel = null; state.ui.msg = ""; state.reviewId = null; persist(); render();
      if (tab === "review" || tab === "mine") reloadSubs(); // always show the current queue
      window.scrollTo(0, 0); return;
    }
    if (t.hasAttribute("data-audit")) { state.auditAll = t.getAttribute("data-audit") === "all"; persist(); renderPulse(); return; }
  ...
  function onChange(e) {
    if (e.target.id === "year-sel") { if (state.tab === "heatmap") state.histYear = e.target.value; else state.year = e.target.value; state.heatSel = null; persist(); render(); }
    ...
    else if (e.target.id === "entry-audit") { readEntryForm(); updateEntryPreview(); }
    else if (e.target.id === "entry-attach") { var af = e.target.files && e.target.files[0]; if (af) handleAttachUpload(af); e.target.value = ""; }
    else if (e.target.id === "er-defects" || e.target.id === "er-defectarea-sel") { readEntryRow(); renderEntry(); }
    else if (["entry-dataset", "entry-branch", "entry-date"].indexOf(e.target.id) >= 0) { readEntryForm(); readEntryRow(); if (state.entry) state.entry.msg = ""; renderEntry(); }
  }
  ...
  // If a Microsoft sign-in round-trip bounced back with ?sso_error=, show the
  // message on the sign-in modal (unless already signed in), then clean the URL.
  try {
    var ssoErr = new URLSearchParams(window.location.search).get("sso_error");
    if (ssoErr) {
      ...
      if (!session) {
        ensureLogin();
        state.login.mode = "role";
        state.login.err = SSO_ERRORS[ssoErr] || "Microsoft sign-in didn’t complete — please try again.";
        showLoginModal();
      }
    }
  } catch (e) { /* ignore */ }
