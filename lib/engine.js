...
export function initTracker(opts) {
  ...
  var state = {
    ...
    entry: null, subs: null, subsErr: "", reviewId: null, reviewFilter: { dataset: "all", branch: "all", sort: "oldest" }, login: null,
  };
  var savedYear = null;
  ...
  function rolePwManagerHtml() {
    ...
    return '<div class="h2l" style="margin-top:18px">Role sign-in passwords</div>' +
      '<div class="card" style="margin-bottom:10px"><div class="sub13" style="margin-bottom:6px">Staff sign in by role from the gear: they pick Inspector or Supervisor, type their name, choose a branch and enter the shared password for that role. Set those shared passwords here — they’re hashed on the server. Until you set one, the admin password works.</div>' +
        (acct.status ? statusRow("inspector", "Inspector") + statusRow("supervisor", "Supervisor") : "") + loadNote + msgHtml + "</div>";
  }
  ...
  var LOGIN_ROLES = [{ id: "inspector", name: "Inspector", desc: "Key branch reviews" }, { id: "supervisor", name: "Supervisor", desc: "Review & sign-off" }];
