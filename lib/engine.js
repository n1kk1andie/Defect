...
export function initTracker(opts) {
  ...
  // Which tabs appear in the bar: public tabs and the Settings login door are always
  // shown; role screens appear only when the signed-in role is allowed to see them.
  function visibleTabs() { return TABS.filter(function (t) { return t.roles ? roleCanSee(t) : true; }); }
  function renderTabs() {
  ...
  async function handleAction(a, el) {
    var arg = el ? el.getAttribute("data-user") : null;
    ...
    if (a === "logout") { await api.logout(); if (!roleCanSee(tabById(state.tab))) state.tab = "pulse"; state.acct = null; render(); return; }
