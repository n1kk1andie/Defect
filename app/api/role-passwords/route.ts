...
async function requireAdmin() {
  return (await isAdmin(Date.now())) ? null : NextResponse.json({ ok: false, error: "Admin sign-in required." }, { status: 401 });
}
...
export async function POST(req: NextRequest) {
  ...
  if ((password || "").trim().length < 12) return NextResponse.json({ ok: false, error: "Password must be at least 12 characters." }, { status: 400 });
